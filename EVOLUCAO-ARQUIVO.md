# Evolução do projeto — arquivo histórico

Sessões antigas, tiradas de `EVOLUCAO.md` para o diário caber numa
leitura. **Ninguém lê este arquivo por padrão** — ele existe para quando
alguém precisar reconstruir por que uma decisão foi tomada. O diário vivo,
com o estado atual e os próximos passos, continua em `EVOLUCAO.md`.
